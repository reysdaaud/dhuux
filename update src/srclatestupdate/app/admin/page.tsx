...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading) { // Only proceed when authentication status is determined
      if (user) { // Check if a user is logged in
        // Check if the authenticated user is an admin
        if (!user.isAdmin) {
          console.log('[AdminPage] User is not an admin, redirecting to home.');
          router.replace('/'); // Redirect to home page if not admin
        } else {
        fetchItems();
        }
  ...
  // Check if user is an admin after auth loading is complete
  // This provides a brief loading state while the isAdmin check is implicit in the useEffect.
  // A more explicit check here could also be added if needed, but useEffect handles the primary logic.
  if (!user || !user.isAdmin) {
     // The useEffect should handle the redirection, so we might not even reach here if redirected.
     // However, keeping a basic message or loader ensures something is displayed if redirection is slow or fails.
     return null; // Or return a loading spinner or a "Access Denied" message if preferred
  }
